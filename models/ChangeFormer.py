...
class DecoderTransformer_v3(nn.Module):
    ...
    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True, 
        ...
        desired_input = int(self.embedding_dim*len(in_channels))
        self.linear_fuse = nn.Sequential(
            nn.Conv2d(   in_channels=desired_input, out_channels=self.embedding_dim,
                                        kernel_size=1),
...
class MLPDecoder(nn.Module):
    ...
    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True, 
        ...
        self.linear_c3 = MLP(input_dim=c3_in_channels, embed_dim=self.embedding_dim) #320, 128
        ...
        self.linear_fuse = nn.Sequential(
            nn.Conv2d(in_channels=desired_in_channels, out_channels=self.embedding_dim,
                                        kernel_size=1),
            nn.BatchNorm2d(self.embedding_dim)
        )
...
class ChangeFormerV6(nn.Module):
    ...
    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False, embed_dim=256, patch_size=2):
        ...
        #Transformer Decoder
        self.TDec_x1   = DecoderTransformer_v3(input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=False, 
                    in_channels = self.embed_dims, embedding_dim= self.embedding_dim, output_nc=output_nc, 
                    decoder_softmax = decoder_softmax, feature_strides=[2, 4, 8, 16])
