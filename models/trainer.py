...
class CDTrainer():
    ...
    def network_sum(self, args):

        model = define_G(args=args, gpu_ids=args.gpu_ids).to(self.device)
        input1 = torch.randn(8,3,256,256).to(self.device)
        input2 = torch.randn(8,3,256,256).to(self.device)

        # flops, params = profile(model, inputs=(input1,input2))
        # flops_Gb = clever_format([flops], "%.2f")
        # params_M = clever_format([params], "%.2f")
        # print(f'FLOPS (Gb): {flops_Gb}, PARAMS (M): {params_M}')
